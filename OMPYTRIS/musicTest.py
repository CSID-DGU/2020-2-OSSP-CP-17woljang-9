...
def setframerate(soundfile):
    fr = wave.open(soundfile, 'wb') # 쓰기 전용 모드로 wav 파일 열기
    fr.setframerate(10) # 프레임 속도를 10으로 설정
    # fr.setframerate(framerate) wav 프레임 속도에 게임 속도가 반영
...
try:
    initMixer()
    filename='assets/sounds/SFX_BattleMusic.wav'
    setframerate(filename)
    playmusic(filename)
